...
from model.FewShot import FewShot as myModel
# options to init
...
with tf.Session(config=config) as sess:
    ...
    for iEpoch in range(epoch_start, opt.nEpoch):
        ...
        if int(iEpoch/opt.nEpoch_state_update)%2==0:
        ...
        for step in range(opt.nStep_train):
            ...
            feed_dict={myFS.input_node_train: _input_ACSk, myFS.target_node_train: _target_ACSk, myFS.input_node_test: _input_k, myFS.target_node_test:_target_k}
        ...
        for step in range(opt.nStep_valid):
            myFS.restore_state(sess)
            _input_ACSk, _target_ACSk, _input_k, _target_k = DB_valid.getBatch(step*opt.batchSize, (step+1)*opt.batchSize)
            feed_dict = {myFS.input_node_train: _input_ACSk, myFS.target_node_train: _target_ACSk, myFS.input_node_test: _input_k, myFS.target_node_test:_target_k}
            if step==0:
