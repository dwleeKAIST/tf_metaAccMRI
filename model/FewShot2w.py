...
class FewShotG:
    def __init__(self, opt, metaLearner, Learner):
        ...
        with tf.device(str_):
            ...
            self.w   = tf.placeholder(dtype,[nB, opt.nCh_in,  opt.nY, opt.nX])
            self.uw  = tf.placeholder(dtype,[nB, opt.nCh_in, opt.nY, opt.nX])
...
#            
            ...
            target_train_k  = tf_pad0(self.myD2C_train.CH2D_(self.input_node_train,self.target_node_train),padX=opt.hPad)*self.uw
            netoutL_train_k = tf_pad0(self.myD2C_train.CH2D_(self.input_node_train,self.net_out_train),padX=opt.hPad)*self.uw
            target_test_k   = self.myD2C_test.CH2D_(self.input_node_test,self.target_node_test)*self.uw
            netout_test_k   = self.myD2C_test.CH2D_(self.input_node_test,self.net_out_test)*self.uw
